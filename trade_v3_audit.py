...
MM_MIN_VOLUME    = int(os.environ.get("MM_MIN_VOLUME", "20"))
MM_ORDER_TAG     = "mm_v1"
...
def apply_phase_limits(phase_num, phase_cfg):
    ...
    # Phase 1: force paper trading regardless of env var
    # Phase 2+: enable live trading (override DRY_RUN from env)
    if phase_num == 1:
        DRY_RUN = True
    elif phase_num >= 2:
        DRY_RUN = False
...
_RATE_LIMITS = {
    ...
    "bls.gov":      (2.0,  2),   # BLS API (free tier, be polite)
}
...
def _cached_get(key, url, timeout=5, headers=None):
    """GET with in-memory cache and per-domain rate limiting."""
    now = time.time()
...
def _get_fed_rate_expectations():
    ...
    cached = _cached_get(cache_key, None, timeout=0)  # check cache only
    if cached and isinstance(cached, dict) and cached.get("_ts", 0) > time.time() - 14400:
        return cached
...
def get_fred_estimate(ticker, market_data):
    ...
    if not thresh_match:
        ...
        if tick_match and indicator == "fed_funds":
            threshold = float(tick_match.group(1))
            is_above = True
        else:
            return None, None
...
def get_cleveland_fed_nowcast(ticker, market_data):
    ...
    # Try to fetch Cleveland Fed nowcast — try multiple known endpoints
    ...
    for i, url in enumerate(clevfed_urls):
        ...
        if data:
            break
    if not data:
        # All Cleveland Fed endpoints failed — source is likely down/moved
        return None, None
...
def get_news_sentiment(ticker, market_data):
    """Check Finnhub news sentiment — ONLY for markets that reference real stock tickers.
    The Finnhub news-sentiment endpoint expects stock symbols (AAPL, TSLA), not
    arbitrary keywords. For weather/sports/crypto/economics markets, this returns noise."""
...
def record_settlements(conn):
    ...
    for s in settlements:
        ...
        if mm_row:
            try:
                conn.execute(
                    "UPDATE mm_inventory SET realized_cents = realized_cents + ? WHERE ticker=?",
                    (profit, ticker))
            except Exception:
    ...
    try:
        ...
        for (source,) in sources:
            recent = conn.execute(
                ...
                "WHERE source = ? ORDER BY id DESC LIMIT 8",
                (source,)
            ).fetchall()
            # Only count actual failures (broken/degraded), not idle
            failure_count = sum(1 for r in recent if r[0] in ("broken", "degraded"))
            if len(recent) >= 8 and failure_count >= 7:
                # Recovery window: every 5 runs, give disabled sources another chance
                if total_runs % 5 == 0:
...
def score_market(m, adaptive_weights=None, calibration_corrections=None, category_edges=None,
    ...
    if fee_adjusted_edge_yes > required_edge and spread < 0.08:
        ...
        detail = (f"info_edge: {info_source} indep={indep_prob:.2f} "
                  ...
                  f"days={days:.1f if days else '?'} time_mult={time_multiplier:.2f}")
...
def track_fills(conn):
    ...
    for o in orders:
        ...
        conn.execute(
            "UPDATE trades SET fill_status=? WHERE order_id=? AND fill_status IS NULL",
            (status, oid))
...
def mm_check_fills(conn):
    ...
    # Add fee_cents column if upgrading from old schema
    try:
        conn.execute("SELECT fee_cents FROM mm_processed_fills LIMIT 1")
    except Exception:
        try:
            conn.execute("ALTER TABLE mm_processed_fills ADD COLUMN fee_cents REAL DEFAULT 0")
        except Exception:
            pass
    ...
    try:
        ...
        while True:
            ...
            for f in api_fills:
                ...
                conn.execute("INSERT OR IGNORE INTO mm_processed_fills VALUES (?, ?, ?)",
                            (fill_id, datetime.now(timezone.utc).isoformat(), fee_cents))
                conn.execute(
...
def mm_select_markets(markets, conn, balance_cents, category_edges=None):
    ...
    filter_stats = {"total": 0, "cat_skip": 0, "no_ask": 0, "no_bid_narrow": 0,
                    ...
                    "inv_full": 0, "expiring": 0, "passed": 0}
    for m in markets:
...
def mm_check_series_profitability(conn, ticker, proposed_side, proposed_qty, proposed_price):
    ...
    try:
        ...
        for t, side, qty, avg_p in dir_rows:
            ...
            if t not in positions:
                net = int(qty) if side == "yes" else -int(qty)
                positions[t] = (net, float(avg_p))
    except Exception as e:
    ...
    try:
        fee_rows = conn.execute(
            """SELECT COALESCE(SUM(pf.fee_cents), 0) FROM mm_processed_fills pf
               INNER JOIN mm_orders mo ON pf.fill_id IN (
                   SELECT fill_id FROM mm_processed_fills
               ) WHERE mo.ticker LIKE ?""",
            (series_prefix + "%",)
        ).fetchone()
        if fee_rows and fee_rows[0]:
            actual_fees_cents = float(fee_rows[0])
    except Exception:
...
def mm_post_quotes(conn, m, fair_value_cents, balance_cents, inventory):
    ...
    if buy_size > 0 and abs(inventory) < MM_MAX_INVENTORY:
        order_body = {
            ...
            "client_order_id": f"mm_bid_{ticker}_{int(time.time())}_{uuid.uuid4().hex[:8]}",
            "post_only": True,
        }
        try:
            if not MM_DRY_RUN:
    ...
    if sell_size > 0 and abs(inventory) < MM_MAX_INVENTORY:
        ...
        order_body = {
            ...
            "action": "buy",
            "expiration_ts": int(time.time() + 110),
            "client_order_id": f"mm_ask_{ticker}_{int(time.time())}_{uuid.uuid4().hex[:8]}",
            "post_only": True,
        }
        try:
            if not MM_DRY_RUN:
...
def mm_run(conn, markets, balance_cents, portfolio_value, adaptive_weights=None,
    ...
    for score, m, ticker, spread, mid, _stale_inventory, cat in mm_candidates:
        ...
        if not has_real_source:
            ...
            if fair_value_cents <= 3 or fair_value_cents >= 97:
...
def mm_liquidate_expiring(conn):
    """Check for MM inventory in markets that are about to expire (<1h).
    Post aggressive exit orders to avoid holding through settlement.
    ...
    try:
        ...
        for ticker, net, avg_entry in inv_rows:
            ...
            try:
                ...
                qty = abs(net)
                if net > 0:
                    # We're long YES — sell by buying NO at a generous price
                    ...
                    no_price = max(1, 100 - yes_bid + 2)  # cross the spread by 2¢
                    order_body = {
                        "ticker": ticker, "side": "no", "type": "limit",
                    ...
                    # We're short YES (long NO) — buy YES to flatten
                ...
                if not MM_DRY_RUN:
                    ...
                    print(f"[mm] Liquidating {ticker}: {qty} contracts "
                          f"(net={net:+d}, {hours_left:.1f}h left)")
                    liquidated += 1
                else:
...
def main():
    ...
    if not ok:
        ...
        try:
            inv_rows = conn.execute(
                "SELECT SUM(ABS(net_contracts) * avg_entry_cents) FROM mm_inventory"
            ).fetchone()
